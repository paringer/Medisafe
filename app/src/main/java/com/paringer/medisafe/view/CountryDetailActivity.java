...
/**
 * An activity representing a single CountryItem detail screen. This
...
public class CountryDetailActivity extends AppCompatActivity {
    ...
    @BindView(R.id.country_detail_container)
    ViewGroup container;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fab.setOnClickListener(this::startMapsActivity);
    ...
    private void startMapsActivity(View view) {
        MyDetailViewModel model = ViewModelProviders.of(this).get(MyDetailViewModel.class);
        LiveData<CountryItem> country = model.getCountry("");
        Intent intent = new Intent(getApplicationContext(), MapsActivity.class);
        CountryItem countryItem = country.getValue();
        if (countryItem == null) return;
        String name = countryItem.getName();
        List<Float> latlng = countryItem.getLatlng();
        if(latlng != null && latlng.size() > 1){
            intent.putExtra("latitude", latlng.get(0));
            intent.putExtra("longitude", latlng.get(1));
